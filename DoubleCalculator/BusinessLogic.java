...
public class BusinessLogic {
	...
	**********************************************************************************************/
	...
	// These are the major calculator values 
		...
		private CalculatorValue err_operand1 = new CalculatorValue(0.0);
		private CalculatorValue err_operand2 = new CalculatorValue(0.0);
		...
		**********************************************************************************************/
		...
		public boolean set_Err_Operand1(String value) 
		{

			err_operand1Defined = false;							// Assume the operand will not be defined
			if (value.length() <= 0) {						// See if the input is empty. If so no error
				err_op1ErrorMessage = "";					// message, but the operand is not defined.
				return true;									// Return saying there was no error.
			}
			err_operand1 = new CalculatorValue(value);			// If there was input text, try to convert it
			err_op1ErrorMessage = err_operand1.getErrorMessage();	// into a CalculatorValue and see if it
			if (err_op1ErrorMessage.length() > 0) 			// worked. If there is a non-empty error 
				return false;								// message, signal there was a problem.
		...
		public boolean set_Err_Operand2(String value) {			// The logic of this method is exactly the
			...
			err_op2ErrorMessage = err_operand2.getErrorMessage();
			if (err_op2ErrorMessage.length() > 0)
				return false;
		...
		public boolean set_Err_Result(String value) {				// The logic of this method is similar to
			if (value.length() <= 0) {						// that for operand1, above.
				err_op2ErrorMessage = "";
				return true;
			}
		...
		public void set_Err_Op1ErrorMessage(String m) 
		{
			err_op1ErrorMessage = m;
			return;
		}
		/**********
		...
		public String get_Err_Op1ErrorMessage() {
			return err_op1ErrorMessage;
		}
		...
		public void set_Err_Op2ErrorMessage(String m) {
			err_op2ErrorMessage = m;
			return;
		}
		...
		public String get_Err_Op2ErrorMessage() {
			return err_op2ErrorMessage;
		}
		...
		public void set_Err_ResultErrorMessage(String m) {
		...
		public String get_Err_ResultErrorMessage() {
		...
		**********************************************************************************************/
		...
		public String Err_debugToString() {
			...
			r += "     operand1ErrorMessage = " + err_op1ErrorMessage+ "\n";
			r += "     operand1Defined = " + err_operand1Defined+ "\n";
			r += "operand2 = " + err_operand2.toString() + "\n";
			r += "     operand2ErrorMessage = " + err_op2ErrorMessage+ "\n";
		...
		**********************************************************************************************/
		...
		public String error_subtraction() 
		{
			result = new CalculatorValue(err_operand1);
			result.add(err_operand2);
			resultErrorMessage = result.getErrorMessage();
...
//			return Double.toString(Math.round(i2*10000000000.0)/10000000000.0); // this cast is used to round off to 10 decimal places, we can extend or decrease the zeros as per the precision req.
		...
		public String error_multiplication() 
		{
			result = new CalculatorValue(err_operand1);
			result.fraction_Error(operand1,err_operand2,operand2);
			resultErrorMessage = result.getErrorMessage();
			double i2 = Double.parseDouble(result.toString()); // this casts the result to string using the toString() method and then we cast it to Double again to use round operation
			return Double.toString(Math.round(i2*10000000000.0)/10000000000.0); // this cast is used to round off to 10 decimal places, we can extend or decrease the zeros as per the precision req.
		}
		...
		{
			result = new CalculatorValue(err_operand1);
			result.fraction_ErrDivision(operand1,err_operand2,operand2);
			resultErrorMessage = result.getErrorMessage();
			if (resultErrorMessage.length() != 0) // a very intuitive approach to handle divide by zero approach was to use length of error message as the cue
			{
				return "";// since the condition returns empty, another conditional is set up in the Userinterface class to display the error
